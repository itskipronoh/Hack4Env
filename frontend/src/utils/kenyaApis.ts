...
export const KENYA_LOCATIONS = [
  ...
  },
  {
    ...
    population: 1208333,
    mainChallenges: ['Coastal erosion', 'Marine pollution', 'Coral reef degradation']
  },
  {
    name: 'Kisumu',
    ...
    population: 610082,
    mainChallenges: ['Lake Victoria pollution', 'Fishing sustainability', 'Water hyacinth']
  },
  {
    name: 'Nakuru',
    ...
    region: 'Rift Valley',
    population: 570674,
    mainChallenges: ['Lake ecosystem health', 'Flamingo conservation', 'Industrial pollution']
  },
  {
    name: 'Eldoret',
    ...
    region: 'North Rift',
    population: 475716,
    mainChallenges: ['Agricultural sustainability', 'Soil erosion', 'Water conservation']
  },
  {
    name: 'Meru',
    ...
    population: 340000,
    mainChallenges: ['Mount Kenya forest conservation', 'Agricultural adaptation', 'Water scarcity']
  },
  {
    name: 'Malindi',
    ...
    region: 'Coastal Kenya',
    population: 118265,
    mainChallenges: ['Marine conservation', 'Tourism sustainability', 'Mangrove protection']
  },
  {
    name: 'Garissa',
    ...
    population: 119696,
    mainChallenges: ['Drought resilience', 'Pastoral adaptation', 'Water access']
  }
];
...
export const KENYA_FORESTS = [
  {
    ...
    coordinates: { lat: -0.4000, lng: 35.8500 },
  ...
  },
  {
    ...
    threats: ['Agricultural expansion', 'Charcoal production', 'Infrastructure development'],
    restoration: {
      target: 25000,
      completed: 6800,
      progress: 27.2
  ...
  },
  {
    ...
    threats: ['Population pressure', 'Firewood collection', 'Agricultural encroachment'],
    restoration: {
      target: 15000,
      completed: 3200,
      progress: 21.3
  ...
  },
  {
    ...
    coordinates: { lat: -3.3000, lng: 39.9000 },
...
export const getKenyaWeatherData = async (location: string): Promise<any> => {
  try {
    ...
    const metResponse = await axios.get(`${KENYA_MET_API.baseUrl}${KENYA_MET_API.endpoints.weather}`, {
      params: { location }
    });
  ...
  try {
    ...
    const kenyaLocation = KENYA_LOCATIONS.find(loc => loc.name.toLowerCase() === location.toLowerCase());
...
export const getKenyaForestData = async (): Promise<any[]> => {
  try {
    ...
    const response = await axios.get(`${KFS_API.baseUrl}${KFS_API.endpoints.coverage}`);
    if (response.data) {
  ...
  return KENYA_FORESTS.map(forest => ({
    ...
    region: forest.name,
    coverage: forest.coverage,
    change: forest.restoration.progress > 20 ? '+' + forest.restoration.progress.toFixed(1) + '%' : '-' + (30 - forest.restoration.progress).toFixed(1) + '%',
    status: forest.coverage > 70 ? 'Good' : forest.coverage > 50 ? 'Fair' : 'Critical',
...
export const getKenyaAirQuality = async (location: string): Promise<any> => {
  try {
    // Try NEMA API
    const response = await axios.get(`${NEMA_API.baseUrl}${NEMA_API.endpoints.airQuality}`, {
      params: { location }
    });
    if (response.data) {
      return response.data;
  ...
  return {
    location: location,
    aqi: isUrban ? Math.floor(Math.random() * 50) + 80 : Math.floor(Math.random() * 30) + 20, // Urban areas have higher AQI
    pm25: isUrban ? Math.floor(Math.random() * 20) + 25 : Math.floor(Math.random() * 10) + 5,
    pm10: isUrban ? Math.floor(Math.random() * 30) + 35 : Math.floor(Math.random() * 15) + 10,
    no2: isUrban ? Math.floor(Math.random() * 25) + 15 : Math.floor(Math.random() * 10) + 2,
    status: isUrban ? 'Moderate' : 'Good',
    mainPollutants: kenyaLocation?.mainChallenges || ['Vehicle emissions', 'Industrial activities']
  };
};
